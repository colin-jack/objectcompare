var differencesLogger = require('./differencesLogger'),
    logPropertiesInOnlyOneObject = require('./logPropertiesInOnlyOneObject'),
...
var getConfigIncludingDefaults = function(config) {
    ...
    return config ? _u.defaults(config, defaults) : defaults;
}
...
var compareProperties = function(first, second, parentsPath, config, diffLogger) {
    ...
    _u.each(propertiesInBoth, function(subProperty) {
...
var performRecursiveComparison = function(first, second, propertyPath, config, diffLogger) {
    ...
    if (first instanceof Date && second instanceof Date && first.getTime() !== second.getTime()) {
        diffLogger.logPrimitivesHaveDifferentValue(propertyPath, first, second);
        return;  
    }
...
module.exports = function(first, second, config) {
